...
import com.vanlumban.nasaphotos.ui.screen.home.HomeScreen
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = {
                    Text(
                        text = if (currentRoute?.startsWith("details") == true) {
                            "Photo Details"
                        } else {
                            items.find { it.route == currentRoute }?.title ?: "NASA Photos App"
                        }
                    )
                },
                navigationIcon = {
        ...
        NavHost(
            ...
            composable("details/{photoJson}") { backStackEntry ->
